// Need to fix imported text in Most Popular FOD
...
let eduvariables = [
  ...
  {label: 'Age', value: 'Age', variable: 'age', variables: ['25-34','35-44','45-54','55-64'],
   deaf: ['deaf people in 25-34 age group','deaf people in 35-44 age group','deaf people in 45-54 age group','deaf people in 55-64 age group'], hearing: ['hearing people in 25-34 age group','hearing people in 35-44 age group','hearing people in 45-54 age group','hearing people in 55-64 age people']},
...
// For Education Data
let eduattain = [
  {label: 'All Levels', value: 'All Levels', title: "Education Attainment - High School, Some College, AA, BA, MA, PHD", variable: 'all'},
  {label: 'High School', value: 'High School', title: "High School Attainment or Higher", variable: 'HS diploma'},
...
function Education({colors, justcolor, colorfill}) {
  ...
  const edulistoptions = {
    option: (base, state) => ({
      ...base,
      margin: 0,
      cursor: isDisabled ? 'not-allowed' : 'pointer',
      fontWeight: state.isSelected ? 900: 100,
      background: state.isSelected & isDisabled ? 'transparent' 
                : state.isSelected & !isDisabled ? '#008e8593' 
    ...
    dropdownIndicator: (provided,state) => ({
      ...
      color: state.isDisabled ? '#bfbfbf' : '#0B7373',
      "&:hover": {
        color: state.isDisabled ? '#bfbfbf' : '#0B7373'
      }
    }),
    ...
    menu: (provided, state) => ({
      ...
      position: 'static',
      overflowX: 'hidden',
      border:  state.isDisabled ? '0.5px #828282 solid':'0.5px #0B7373 solid',
      margin:0,
      zIndex: 0,
      padding: 0,
      background: "#F6F6F7",
    ...
    singleValue:(provided, state) => ({
      ...
      height:'100%',
      fontWeight: 700,
      color: state.isDisabled ? 'transparent' :'#0B7373',
    })
  }
  ...
  const [eduLabelDescript, setEduLabelDescript] = useState("bachelor’s degree or higher")
  ...
  }, [content,eduLevel,title_by])

  // Sorting by another array
  const sortarray = employment.filter(employment => employment.type === 'Field of Degree' &
   employment.variable ===  'Graduate' &
   employment.state === 'United States' &
   employment.attribution.includes('deaf')).map(
   employment => employment.status)

  // Effect of Group Buttons
  const [button_group_a, setButtonGroup_a] = useState('Percentages of Graduate Across')
  const [type_a, setType_A] = useState('Graduate')
  const [senttype_a,setSentType_a] = useState('have completed a bachelor’s degree or higher in ')
  ...
  const changeButton_A = (e) => {
    if(e.target.name === 'Employment'){
      setType_A('Employment')
      setButtonGroup_a('Employment Rates By')
      setSentType_a('are employed in ')
      setDGrad(
        employment.filter(employment => employment.type === 'Field of Degree' &
    ...
    }else if(e.target.name === 'Graduate'){
      setType_A('Graduate')
      setButtonGroup_a('Percentages of Graduate Across')
      setSentType_a('have completed a bachelor’s degree or higher in ')
      setDGrad(
        employment.filter(employment => employment.type === 'Field of Degree' &
        employment.variable ===  'Graduate' &
        employment.state === 'United States' &
  ...
  let education = {
    ...
    yAxis: {
      min: 0,
      max: 1+Math.max(...employment.filter(employment => (employment.attribution === attribute[0] |
      ...
      chartOptions: { // specific options for the exported image
        ...
        subtitle: {
          ...
          employment.filter(employment => employment.attribution === attribute[0] & 
            ...
            +
          words[0]+' have completed'+eduA+eduLabelDescript.toLowerCase()+', compared to '+
          ...
          size_checker(employment.filter(employment => employment.attribution === attribute[0] & 
            employment.status === eduLevel & 
            employment.type === 'education' & 
            employment.state === chosen_state).map(employment => employment.n)).toLocaleString('en-US')+' '+
          attribute[0]+' people and '+
          size_checker(employment.filter(employment => employment.attribution === attribute[1] & 
            employment.status === eduLevel & 
            employment.type === 'education' & 
            employment.state === chosen_state).map(employment => employment.n)).toLocaleString('en-US')+
          ' '+attribute[1]+
  ...
  let education_variable_only = {
    ...
    xAxis: {
      ...
      gridLineWidth: 0,
      accessibility:{
        description: 'categories based on '+selected_attributions
      }
    },
    yAxis: {
      min: 0,
      ...
      accessibility: {
        description: 'what percent of group in the population has completed'+eduA+eduLabelDescript
      }
    },
    tooltip: {
      pointFormat: '{categories}<br>{series.name}: <b>{point.y:.1f}%</b></br>',
      ...
      series: {
        ...
        formatter: function () {
          if(
            ...
            employment.variable === selected_attributions & employment.state === chosen_state &
            employment.status === eduLevel & employment.attribution.includes(this.series.name) &
      ...
      chartOptions: { // specific options for the exported image
        ...
        subtitle: {
          ...
          employment.filter(employment => employment.type === 'education' & 
            ...
            (((employment.margin_errors/100)/1.962937)/(employment.percentage/100) > 0.3 ? employment.percentage + '% \u26a0 of ' : employment.percentage + '% of ')+
            deaf_labels[employment.index]})+
          ' have completed'+eduA+eduLabelDescript.toLowerCase()+', compared to '+
          employment.filter(employment => employment.type === 'education' & 
            employment.variable === selected_attributions & employment.state === chosen_state &
            employment.status === eduLevel & employment.attribution.includes('hearing')).map(
          ...
          size_checker(employment.filter(employment => employment.type === 'education' & 
            employment.variable === selected_attributions & employment.state === chosen_state &
            employment.status === eduLevel & employment.attribution.includes('deaf')).map(employment => employment.n).reduce(
              (sum, a) => sum + a, 0)).toLocaleString('en-US')+' deaf people and '+
          size_checker(employment.filter(employment => employment.type === 'education' & 
            employment.variable === selected_attributions & employment.state === chosen_state &
            employment.status === eduLevel & employment.attribution.includes('hearing')).map(employment => employment.n).reduce(
      ...
      chartOptions: { // specific options for the exported image
        ...
        subtitle: {
          ...
          size_checker(employment.filter(employment => (employment.attribution === 'deaf') & 
            employment.status === 'no HS diploma' & employment.type === 'education' & 
            employment.state === chosen_state & employment.attribution.includes('deaf')).map(employment => employment.n).reduce(
              (sum, a) => sum + a, 0)).toLocaleString('en-US')+' deaf people and '+
          size_checker(employment.filter(employment => (employment.attribution === 'hearing') & 
            employment.status === 'no HS diploma' & employment.type === 'education' & 
            employment.state === chosen_state & employment.attribution.includes('hearing')).map(employment => employment.n).reduce(
              (sum, a) => sum + a, 0)).toLocaleString('en-US')+
          ' hearing people who participated '+in_the.toLowerCase()+year+
      ...
      chartOptions: { // specific options for the exported image
        ...
        subtitle: {
          ...
          employment.filter(employment => employment.attribution === attribute[0] & 
            ...
            +
          words[0]+' are enrolled in postsecondary education and training, compared to '+
          employment.filter(employment => employment.attribution === attribute[1] & 
            employment.status === 'Enrolled' &  
            employment.type === 'enrollment' & 
            employment.state === 'United States').map(employment => [employment.margin_errors,employment.percentage]).map(
          ...
          words[1]+'.<br><br>In this chart, estimates are based on a sample size of '+
          size_checker(employment.filter(employment => employment.status === 'Enrolled' &  
            employment.type === 'enrollment' & 
            employment.attribution === attribute[0] & 
            employment.state === 'United States').map(
            employment => employment.n).reduce(
            (sum, a) => sum + a, 0).toLocaleString('en-US'))+' '+words[0]+
          ' people and '+
          size_checker(employment.filter(employment => employment.status === 'Enrolled' &  
            employment.type === 'enrollment' & 
            employment.attribution === attribute[1] & 
          ...
          'who participated '+in_the.toLowerCase()+year+
          ' American Community Survey. The margin of errors are '+
          employment.filter(employment => employment.attribution === attribute[0] & 
            employment.status === 'Enrolled' &  
            employment.type === 'enrollment' & 
  ...
  let enrollment_variable_only = {
    ...
    yAxis: {
      min: 0,
      max: 1+Math.max(...employment.filter(employment => employment.type === 'enrollment' & 
        employment.variable === selected_attributions & employment.state === 'United States' &
        employment.status === 'Enrolled').map(
        employment => employment.percentage)),
      gridLineColor: '#ffffff',
      gridLineWidth: 0,
      title: {
      ...
      accessibility: {
        description: 'Enrollment in postsecondary programs or training'
      }
    },
    tooltip: {
      pointFormat: '{categories}<br>{series.name}: <b>{point.y:.1f}%</b></br>',
    ...
    plotOptions: {
      ...
      series: {
        ...
        formatter: function () {
          if(
            (employment.filter(employment => employment.type === 'enrollment' & 
            employment.variable === selected_attributions & employment.state === 'United States' &
            employment.status === 'Enrolled' & employment.attribution.includes(this.series.name) &
            employment.percentage === this.y).map(
            function(employment){return (((employment.margin_errors/100)/1.962937))})/(this.y/100) > 0.3)
          ){
            return '\u26a0'+this.y + '%'
    ...
    series: [{
      ...
      borderColor: colorfill[0],
      borderWidth: 1,
      data: employment.filter(employment => employment.type === 'enrollment' &
        employment.status === 'Enrolled' &  
        employment.type === 'enrollment' &
      ...
      chartOptions: { // specific options for the exported image
        ...
        subtitle: {
          ...
          employment.filter(employment => employment.status === 'Enrolled' &  
            ...
            (((employment.margin_errors/100)/1.962937)/(employment.percentage/100) > 0.3 ? employment.percentage + '% \u26a0 of ' : employment.percentage + '% of ')+
            deaf_labels[employment.index]})+
          ' are enrolled in postsecondary education and training, compared to '+
          employment.filter(employment => employment.status === 'Enrolled' &  
            employment.type === 'enrollment' & 
            employment.variable === selected_attributions & employment.state === 'United States' &
            employment.attribution.includes('hearing')).map(
          ...
          '.<br><br>In this chart, estimates are based on a sample size of '+
          size_checker(employment.filter(employment => employment.status === 'Enrolled' &  
            employment.type === 'enrollment' & employment.attribution.includes('deaf') &
            employment.variable === selected_attributions & employment.state === 'United States').map(
            employment => employment.n).reduce(
            (sum, a) => sum + a, 0).toLocaleString('en-US'))+
          ' deaf people and '+
          size_checker(employment.filter(employment => employment.status === 'Enrolled' &  
            employment.type === 'enrollment' & employment.attribution.includes('hearing') &
            employment.variable === selected_attributions & employment.state === 'United States').map(
            employment => employment.n).reduce(
            (sum, a) => sum + a, 0).toLocaleString('en-US'))+
          ' hearing people who participated '+in_the.toLowerCase()+year+
  ...
  // Most Popular Fields of Degrees
  let most_popular = {
    chart:{
      type: 'column',
      width: HCwidth,
    ...
    xAxis: {
      ...
      gridLineWidth: 0,
      accessibility:{
        description: 'categories based on top 5 field of degrees'
      }
    },
    yAxis: {
      min: 0,
        ...
        formatter: function () {
          if(
            (employment.filter(employment => employment.type === 'Field of Degree' &
              employment.variable ===  type_a &
              employment.state === 'United States' &
              employment.attribution.includes(this.series.name) &
              employment.percentage === this.y).map(
              function(employment){return (((employment.margin_errors/100)/1.962937))})/(this.y/100) > 0.3)
          ){
            return '\u26a0'+this.y + '%'
      ...
      chartOptions: { // specific options for the exported image
        ...
        subtitle: {
          ...
          dgraduate.map(function(dgraduate, index){ return index === 0 ?
            ...
            dgraduate[0].replaceAll("\n", " ").toLowerCase() : index === 4 ? ' '+
            (((dgraduate[2]/100)/1.962937)/(dgraduate[1]/100) > 0.3 ? dgraduate[1] + "% \u26a0 of deaf people have completed a bachelor’s degree or higher in " :
            dgraduate[1] + "% of deaf people "+senttype_a)+
            dgraduate[0].replaceAll("\n", " ").toLowerCase() :
            ' '+dgraduate[1]+'% in '+dgraduate[0].replaceAll("\n", " ").toLowerCase()}).reverse()+
          ', compared to '+
          hgraduate.map(function(hgraduate, index){ return index === 0 ?
            ...
            hgraduate[0].replaceAll("\n", " ").toLowerCase() : index === 4 ? ' '+
            (((hgraduate[2]/100)/1.962937)/(hgraduate[1]/100) > 0.3 ? hgraduate[1] + "% \u26a0 of hearing people have completed a bachelor’s degree or higher in " : 
            hgraduate[1] + "% of hearing people "+senttype_a)+
            hgraduate[0].replaceAll("\n", " ").toLowerCase() : ' '+hgraduate[1]+'% in '+hgraduate[0].replaceAll("\n", " ").toLowerCase()
            }).reverse()+
            '.<br><br>In this chart, estimates are based on a sample size of '+
            size_checker(employment.filter(employment => employment.type === 'Field of Degree' &
              employment.variable ===  type_a &
              employment.state === 'United States' &
            ...
            ' American Community Survey. The margin of errors are between '+
            Math.min(...employment.filter(employment => employment.type === 'Field of Degree' &
              employment.variable ===  type_a &
              employment.state === 'United States' &
              employment.attribution.includes('deaf')).map(employment => employment.margin_errors))+
              '% and '+
            Math.max(...employment.filter(employment => employment.type === 'Field of Degree' &
              employment.variable ===  type_a &
              employment.state === 'United States' &
              employment.attribution.includes('deaf')).map(employment => employment.margin_errors))+
            '% for deaf people and between '+
  ...
  let all_fods = {
    ...
    xAxis: {
      ...
      gridLineWidth: 0,
      accessibility:{
        description: 'categories based on all fields of degree'
      }
    },
    yAxis: {
      min: 0,
      ...
      <div className="body">
        <div className = 'container'>
          ...
          <Tabs aria-label="A set of charts" onSelect={tabIndex => set_A_Tab(tabIndex)}>
            <TabList>
            <Tab style={{paddingLeft:paddingSide, paddingRight: paddingSide}}>{'Education Attainment'.slice(0,slice_string[0]).trim()+slice_string[1]}</Tab>
            <Tab style={{paddingLeft:paddingSide, paddingRight: paddingSide}}>{'Enrollment Rate'.slice(0,slice_string[0]).trim()+slice_string[1]}</Tab>
            <Tab style={{paddingLeft:paddingSide, paddingRight: paddingSide}}>{'Most Popular Majors'.slice(0,slice_string[0]).trim()+slice_string[1]}</Tab>
            <Tab style={{paddingLeft:paddingSide, paddingRight: paddingSide}}>{'All Fields of Degrees'.slice(0,slice_string[0]).trim()+slice_string[1]}</Tab>
            </TabList>
            <TabPanel>
              <div className='inside_container'>
                ...
                <p className='aria-text'>
                  ...
                  all information in exported chart in the education attainment content.
                </p>
                  ...
                  <div className='data_sidebar_interface' aria-hidden = 'true'>
                    ...
                    <ContentDowndrop buttonLabel = {'More'+more_options+'Options'}
                      ...
                      textContent={
                        ...
                        options = {eduattributions.filter(eduattributions => eduattributions.variable === selected_attributions)}
                        isSearchable = {false}
                ...
                <div className={data_grid}>
                  <div className='a'>
                    <div className = 'title' aria-hidden = 'true'>{eduLevelTitle.toUpperCase()+edutitle_by.toUpperCase()+': '+chosen_state.toUpperCase()}</div>
                    <div className = 'aria-text'>{'Chart Title - '+eduLevelTitle+edutitle_by+': '+chosen_state}</div>
                    ...
                    <form>
                      ...
                      <Select
                        ...
                        defaultValue={{label: eduLabel, value: eduLabel}}
                        options = {eduattain}
                        isSearchable = {false}
                    ...
                    {
                      {
                        accordionBtn: <HighchartsReact highcharts={Highcharts} options={education_variable_only}/>,
                        accordionBtnActive: <HighchartsReact highcharts={Highcharts} options={education}/>,
                        AllLevels: <HighchartsReact highcharts={Highcharts} options={eduall}/>
                      }[chart_ed]
                    }
                    ...
                    <ContentDowndrop buttonLabel = '' 
                      ...
                      n = {
                        {
                          accordionBtn: Math.max(...employment.filter(employment => employment.type === 'education' & 
                            employment.variable === selected_attributions & employment.state === chosen_state &
                            employment.status === eduLevel).map(employment => [employment.margin_errors,employment.percentage]).map(
                  ...
                  <div className='b' style={{display:interface_side}}>
                    ...
                    <p className='aria-text'>
                      ...
                      including all information in exported chart in the education attainment content.
                    </p>
                    <form>
            ...
            <TabPanel>
              <div className='inside_container'>
                ...
                <div className={data_grid}>
                  <div className='a'>
                  <div className = 'title' aria-hidden = 'true'>{'ENROLLMENT RATE'+edutitle_by.toUpperCase()+': UNITED STATES'}</div>
                    <div className = 'aria-text'>{'Chart Title - '+eduLevelTitle+edutitle_by+': United States'}</div>
                    {
                      {
                        accordionBtn: <HighchartsReact highcharts={Highcharts} options={enrollment_variable_only}/>,
                        accordionBtnActive: <HighchartsReact highcharts={Highcharts} options={enrollment}/>
                      }[chart]
                    }
                    <p className='aria-text'>Description:</p>
                    <ContentDowndrop buttonLabel = '' 
                      ...
                      n = {
                        {
                          accordionBtn: Math.max(...employment.filter(employment => employment.type === 'enrollment' & 
                            employment.variable === selected_attributions & employment.state === 'United States' &
                            employment.status === 'Enrolled').map(employment => [employment.margin_errors,employment.percentage]).map(
                              function(ME){return(((ME[0]/100)/1.962937)/(ME[1]/100))}
                            )),
                          accordionBtnActive: Math.max(...employment.filter(employment => (employment.attribution === attribute[0] | 
                            employment.attribution === attribute[1]) & 
                  ...
                  <div className='b' style={{display:interface_side}}>
                    ...
                    <ContentDowndrop buttonLabel = {'More'+more_options+'Options'} 
                      ...
                      <form>
                        ...
                        options = {attributions.filter(eduattributions => eduattributions.variable === selected_attributions)}
                        isSearchable = {false}
                        onChange = {changeAttribution_A}/>
                      </form>}
                    />
            ...
            <TabPanel>
              <div className='inside_container'>
                ...
                <p className='aria-text'>
                  When you select any options, this will change not only a chart but title and description.
                </p>
                <p className='aria-text'>Beginning of Interactive Chart</p>
                <button className = 'data_sidebar_button' ref={buttonRef} onClick= {changeSideBarWidth} style = {{display:data_sidebar}} tabIndex = {tabindex1}>
                  <FontAwesome name = 'caret-left' className = 'icon_style' style={{transform: icon_rotate}}/>
                </button>
                <div className = 'title'>{button_group_a.toUpperCase()+' 5 MOST POPULAR MAJORS: UNITED STATES'}</div>
                <ButtonGroup id = 'employment' thewidth = {'30%'} buttons={[
                  "Graduate", 
                  "Employment"]}
                  AfterClick = {changeButton_A}
                />
                <HighchartsReact highcharts={Highcharts} options={most_popular}/>
                <ContentDowndrop buttonLabel = '' 
                  clickedId = {clickedId1}
                  symbol = {symbol1}
                  content = {content1}
            ...
            <TabPanel>
              <div className='inside_container'>
                ...
                <div className = 'title'>{button_group_a.toUpperCase()+' FIELD OF DEGREE: UNITED STATES'}</div>
                <ButtonGroup id = 'employment' thewidth = {'30%'} buttons={[
                  "Graduate", 
                  "Employment"]}
                  AfterClick = {changeButton_A}
                />
                <HighchartsReact highcharts={Highcharts} options={all_fods}/>
              </div>
            </TabPanel>
          </Tabs>
        </div>
